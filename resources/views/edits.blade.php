            ...
            <div class="col-md-12">
                ...
                <div class="card">
                    <div class="card-header">
                        <h4 class="m-0">
                            <a href="{{ url('savings') }}" class="btn btn-primary float-end">Back</a>
                        </h4>
                    </div>
                    <div class="card-body">
                        <form action="{{ url('savings/'.$saving->id) }}" method="POST">
                            @csrf
                            @method('PUT')

                            <div class="mb-3">
                                <label for="exp">Savings</label>
                                <input type="text" id="exp" name="exp" value="{{ $saving->amount }}" required>
                                @error('exp') <span class="text-danger">{{ $message }}</span> @enderror
                            </div>

                            <div class="mb-3">
                                <label for="price">Price</label>
                                <input type="text" id="price" name="price" value="{{ $saving->date }}" required>
